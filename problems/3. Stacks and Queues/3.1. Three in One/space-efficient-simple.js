...
Third stack starts at 1/3 of max size of array Math.round(maxSize / 3)
When stacks are about to overlap, move middle stack to either side (whichever still has unused space)
...
Drawbacks:
can become slow - once the stacks start to overlap, we have to move the middle stack left or right
...
function TripleStack(maxSize) {
    ...
    this.middleStart = Math.round(maxSize / 3);
    this.leftSize = 0;
    this.middleSize = 0;
    this.rightSize = 0;
}
...
TripleStack.prototype.push = function(stackNumber, val) {
    ...
    if (stackNumber === 0) {
        if (this.leftSize === this.middleStart) {
    ...
    if (stackNumber === 1) {
        const desiredIndex = this.maxSize - 1 - this.rightSize;
        if (desiredIndex === this.middleStart + this.middleSize - 1) {
            this.moveMiddle('left');
        }
        this.array[desiredIndex] = val;
        this.rightSize++;
    }

    if (stackNumber === 2) {
        const desiredIndex = this.middleStart + this.middleSize;
        if (desiredIndex === this.maxSize - this.rightSize) {
            this.moveMiddle('left');
        }
        this.array[desiredIndex] = val;
        this.middleSize++;
...
TripleStack.prototype.moveMiddle = function(direction) {
    const start = this.middleStart;
    const end = this.middleStart + this.middleSize - 1;
    if (direction === 'right') {
        // only move values if there are any
...
/**
 * Peek at the top item of a specific stack
